...
const LikedVideos = () => { // Keeping component name matching file name for consistency, though it handles Content
  ...
  const fetchItems = async () => {
    try {
      ...
      if (contentType === 'video') {
         ...
         const response = await photoAPI.getLiked();
         fetchedItems = response.data.photos;
      }

      setItems(fetchedItems);
    } catch (error) {
      console.error(`Error fetching liked ${contentType}s:`, error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Liked Content</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-900 rounded-lg shadow dark:shadow-gray-900/10 dark:border dark:border-gray-800">
          <p className="text-gray-500 dark:text-gray-400 text-lg">You haven't liked any {contentType}s yet.</p>
        </div>
      ) : (
